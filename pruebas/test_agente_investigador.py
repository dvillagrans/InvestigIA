...
Prueba aislada del Agente Investigador:
    python pruebas/test_agente_investigador.py
"""
from agentes.agente_investigador import ejecutar
from contratos import EvidenciaRecuperada
from fixtures import ESQUEMA_GUION
...
def main() -> None:
    ...
    print("estado['evidencia']:")
    print(f"  {estado_salida['evidencia']}")

    imprimir_separador("VALIDACION RAPIDA")
    campo_propio = estado_salida["evidencia"]
    if campo_propio is None:
        print("PROBLEMA: el campo de salida quedo como un None.")
    else:
        print(f"OK -- el campo de salida quedo lleno, tipo: {type(campo_propio).__name__}")
        print(f"  fragmentos recuperados: {len(campo_propio.fragmentos)}")
        for fragmento in campo_propio.fragmentos:
            print(f"  - {fragmento.id}: {fragmento.titulo[:60]}...")
